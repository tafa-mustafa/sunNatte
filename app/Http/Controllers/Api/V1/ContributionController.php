...
class ContributionController extends Controller
{
    ...
    {
        $user = auth()->user();
        $montant = 10;// round($tontine->montant / $tontine->nombre_personne, 2);
    ...
    public function confirmPayment(Request $request, Tontine $tontine)
    {
        ...
        /** $tontine_id_from_session = $payment_status['tontine_id'] ?? null;  // Récupérer le tontine_id de la session de paiement

         // Vérifier que le tontine_id correspond à celui du paramètre
         if ($tontine->id !== (int)$tontine_id_from_session) {
             return response()->json(['error' => 'Le tontine_id de la session ne correspond pas à cette tontine.'], 422);
         }
     */
        ...
        if ($status === 'succeeded') {
            ...
            try {
                ...
                $tontine->contributions()->create([
                    'user_id' => $user->id,
                    'tontine_id' => $tontine->id, // S'assurer que la contribution est associée à cette tontine
                    'montant' => $amount,
